...
async fn main() -> anyhow::Result<()> {
    ...
    let (req_body, timestamp, sign, client_sign) = sdk.create_wallet(open_id, chain_id)?;

    let client = Client::new();
    let url = "https://sandbox-api.privatex.io/sdk/wallet/create";
    ...
    match rsa_utils::to_string_map(&body) {
        Ok(map_data) => {
            match serde_json::from_str::<pay_sdk_rust::response_define::ResponseCreateWallet>(&body) {
                Ok(rsp_data) => {
                ...
                Err(e) => eprintln!("parse JSON ResponseCreateWallet failed: {:?}", e),
            }
        }
