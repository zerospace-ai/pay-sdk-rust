use pay_sdk_rust::{
    response_define::{ResponseCommon}, rsa_utils, sdk::{load_config, SDKConfig, Sdk}
};
use reqwest::Client;
use tokio;
...
async fn main() -> anyhow::Result<()> {
    ...
    let open_id = "HASH202509061500";
    let chain_ids = cfg.chain_ids.as_str();

    let (req_body, timestamp, sign, client_sign) = sdk.get_wallet_addresses(open_id, chain_ids)?;

    let client = Client::new();
    let url = "https://sandbox-api.privatex.io/sdk/wallet/getWalletAddresses";
